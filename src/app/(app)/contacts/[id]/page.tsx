...
import { MobileCollapseHeader } from "@/components/ui/mobile-collapse-header"
...
export default async function ContactDetailPage({ params, searchParams }: PageProps) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      {/* Desktop: the iOS contact-card header — circular back in the corner,
          name centered at the top edge, quick actions centered below. Hidden
          below md, where the collapsing mobile header (inside the scroll
          region) takes over. */}
      <div className="hidden md:block shrink-0 px-4 md:px-8 pt-4 md:pt-6 pb-4 bg-bg max-w-3xl w-full mx-auto">
        <div className="grid min-h-11 grid-cols-[1fr_auto_1fr] items-center gap-x-[var(--space-sm)]">
          ...
          <h1 className="min-w-0 truncate text-center font-display text-heading text-ink leading-[var(--leading-snug)] tracking-[var(--tracking-tight)] font-semibold">
            {displayName}
          </h1>
          <span aria-hidden />
        </div>
        <StatusFlags contact={contact} className="mt-1.5" />
        <QuickActions contact={contact} voiceConfigured={voiceConfigured} editHref={editHref} className="mt-5" />
      </div>
      ...
      >
        {/* Mobile: iOS large-title chrome — the name collapses into a blurred
            pinned bar as the grouped list scrolls under it. */}
...
// Colour-coded status line (member / opt-out / language) under the name. Shared
// by the desktop card header and the mobile collapsing header so the two never
// drift; renders nothing when the contact has no flags.
function StatusFlags({
  ...
  return (
    <div
      className={cn(
        "flex flex-wrap items-center justify-center gap-2.5 text-label font-semibold uppercase tracking-[var(--tracking-wide)] leading-none",
        className,
      )}
    >
...
// The circular quick-action row (Message / Call / Email / Edit). Shared by both
// headers; greys out a channel that has nothing to act on.
function QuickActions({
  ...
  contact: { id: string; phone: string | null; email: string | null; name: string | null }
