...
import { PRAYER_STATUS_ORDER, PRAYER_STATUS_META, type PrayerStatus } from "./status"
...
export function PrayerActions({
  ...
  async function sendEncouragement() {
    ...
    setSending(true)
    try {
      const res = await fetch(`/api/prayer/${id}/encourage`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        ...
        toast.success(j?.mock ? "Recorded (no SMS provider configured)" : "Encouragement sent")
        setBody("")
        setComposing(false)
        router.refresh()
      }
  ...
  return (
    <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-ink-hairline pt-3">
      <label className="inline-flex items-center gap-1.5">
        ...
        >
          {PRAYER_STATUS_ORDER.map((s) => (
            <option key={s} value={s}>
              {PRAYER_STATUS_META[s].label}
            </option>
          ))}
        </select>
      </label>
      ...
      {canText && !composing && (
        <Button variant="ghost" size="sm" onClick={() => setComposing(true)}>
          <Send size={14} /> Send encouragement
        </Button>
      )}
      ...
      {composing && (
        <div className="w-full mt-2">
          <Textarea
            ...
            rows={3}
            placeholder={contactName ? `A short note to ${contactName}…` : "A short note…"}
          />
          <div className="mt-2 flex items-center gap-2">
            <Button size="sm" onClick={sendEncouragement} disabled={sending || !body.trim()}>
          ...
          <p className="mt-1 text-micro text-ink-faint">
            Sent as a transactional reply to their prayer request. STOP is always honored.
          </p>
        </div>
      )}
    </div>
