...
SONG STARTS depend on the song's kind, so do not frame them all the same way. Decide the kind first, then anchor accordingly.

WORSHIP songs are the regular congregational songs, introduced by someone naming what the church is about to sing ("for our first song we'll be singing Love Lifted Me," "let's all stand and sing"). The band is already in place, so the music begins promptly once the announcer stops speaking, and the wordless stretch between the end of that announcement and the first sung word is the instrumental intro. That intro is the song. So anchor a worship song's start to the moment the announcer finishes speaking, NOT to the first sung word. A caption's timestamp marks where its words begin, not where they end, so allow a few seconds for the spoken sentence to finish, then start the clip just after it. This keeps the whole intro and lets none of the announcement be heard. When unsure, sit closer to the end of the announcement than to the singing: keeping a breath of intro is far better than clipping it.

PROGRAM songs (special or performed music) are introduced by announcing a person to come and sing ("our sister has prepared a song," "brother Daniel will sing, please come up"). Here a gap usually follows the announcement while the performer walks up and gets ready, and that gap is genuine dead space, not music. So do NOT anchor a program song to the end of the announcement: that opens the clip on shuffling, a walk-up, or the tail of the speaker's voice. Instead anchor backward from the music. Find the first sung word, step back by the length of the song's own intro to find where the performance truly begins, and start a touch before that. Leave the announcement and the walk-up in the previous chapter. If you can identify the song, use what you know about how it opens (a long piano intro, a short pickup, an a cappella first line) to judge how far back to step; if you cannot, assume a short intro and step back only a few seconds from the first sung word. A listener who taps a program song should land on the first note of the performance, never on the announcement and never on dead air.

For both kinds, let the end be the last note ringing out, with the same grace given to the intro.

READING THE SILENCES: the captions rarely describe the music, so reason from the gaps. A wordless gap between a spoken introduction and the first sung line is almost always intro (worship) or walk-up plus intro (program), never silence to be trimmed. The wording of the introduction tells you the kind: "we'll all sing" points to worship, "is going to sing for us" points to program. Once you have named a song, your knowledge of how it opens is a legitimate clue for placing its start, used to refine the estimate, not to override what the timestamps plainly show.
