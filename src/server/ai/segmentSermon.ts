...
- A song is the whole performance. Let it begin as the music lifts (its instrumental intro) and end as it settles (the last note ringing out). The [music] cues hint at where music is playing when words are sparse, so lean on them and reason about where the song truly starts and stops. Be conservative at the front: once you have judged where the intro begins, set the start about a second ahead of it rather than right on it. Clipping the first beat of a song is far worse than a breath of lead-in, so whenever you are unsure, start a touch early. Give the end the same grace, letting the final note fully ring out before the boundary.
