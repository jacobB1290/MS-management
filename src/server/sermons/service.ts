...
export async function runSermonPipeline(opts: RunOptions): Promise<RunResult> {
  ...
  let transcriptText = sermon.transcript
  let timestamped: string | null = null
  let durationSec = sermon.duration_sec ?? 0
  const needTranscript = opts.force || !transcriptText
  if (needTranscript) {
    if (!hasCaptionAccess()) {
    ...
    transcriptText = res.transcript.plainText
    timestamped = res.transcript.timestamped
    durationSec = res.transcript.durationSec
    ...
    // Reusing a stored transcript: rebuild a lightweight timestamped view is not
    // possible (we only kept plain text), so segment from plain text.
    timestamped = transcriptText
    await rec.finish("transcribe", "skipped", { detail: "reused stored transcript" })
  }
